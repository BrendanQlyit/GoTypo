...
func printDescription(){
	...
	for _, target := range targets {
		...
		for _, t := range []Target{
			{"Additional Char", sanitizedDomain, extraCharAttack},
			{"Bitsquatting", sanitizedDomain, bitsquattingAttack},
			{"Character Swap", sanitizedDomain, swapAttack},
			{"Duplication", sanitizedDomain, duplicationAttack},
			{"Fat Finger", sanitizedDomain, FFAttack},
			{"Homograph", sanitizedDomain, homographAttack},
			{"missing dot", sanitizedDomain, missingDot},
			{"Omission", sanitizedDomain, omissionAttack},
			{"TLD", sanitizedDomain, tldAttack}} {
...
func main (){
	...
	if option > 5 || option <=0{
		for option > 5 || option <=0{
			fmt.Printf("Invaild Option")
			println("")
			getOption()
		}
	}
